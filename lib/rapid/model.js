...
Model.prototype.addProperty = function(name, val){
    ...
    var options = this.properties.get(name);
    // TODO: abstract
    switch (options.type) {
        case 'String':
            val = val.toString();
...
Model.prototype.hasProperty = function(name){
    return this.constructor.properties.get(name);
};
...
Model.prototype.saveProperties = function(fn){
    var pending = 0;
    this.properties.each(function(val, name, options){
        ++pending;
...
Model.prototype.inspect = function(){
    return '<' + this.modelName + ':' + this.id 
        + (this.stale ? ' stale' : '')
        + (this.new ? ' new' : '')
        + this.properties.inspectProperties()
        + '>';
};
...
exports.classMethods = {
    ...
    addProperty: function(name, options){
        this.properties.set(name, new Property(name, options));
